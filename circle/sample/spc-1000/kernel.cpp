...
TShutdownMode CKernel::Run (void)
{
	...
	if (pKeyboard == 0)
	{
		...
		m_Screen.Write ("keyboard not found\n", 19);
...
///				m_Screen.Write ("6847 update\n", 12);	// jinsmac add show we are alive
///				m_ActLED.Blink (1);	// jinsmac add show we are alive

///				m_GUI.Update();
				m_Timer.usDelay(50000);

///				m_Screen.Write ("_GUI update\n", 12);	// jinsmac add show we are alive
///				m_ActLED.Blink (1);	// jinsmac add show we are alive

				//R->ICount -= 20;
			}
///			m_Screen.Write ("8910 upstart\n", 13);	// jinsmac add show we are alive
///			m_ActLED.Blink (1);	// jinsmac add show we are alive
			ay8910.Loop8910(&spcsys.ay8910, 1);
///			m_Screen.Write ("8910 update\n", 12);	// jinsmac add show we are alive
///			m_ActLED.Blink (1);	// jinsmac add show we are alive
...
/*
			...
			m_Screen.Write ("cas read\n", 9);	// jinsmac add show we are alive
			m_ActLED.Blink (1);	// jinsmac add show we are alive
			}
			else
			{
				spcsys.cas.read = 0;
			m_Screen.Write ("cas not read\n", 13);	// jinsmac add show we are alive
			m_ActLED.Blink (1);	// jinsmac add show we are alive
			}
*/
